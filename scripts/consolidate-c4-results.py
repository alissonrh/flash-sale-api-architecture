...
def validate_required_files(runs: list[Path]) -> None:
    ...
    run_one_previous_gateway_logs = list(
        (OFFICIAL_RUNS_DIRECTORY / "c4-run-1/logs/previous").glob("gateway-*-gateway.log")
    )
    if not run_one_previous_gateway_logs:
        missing.append("c4-run-1/logs/previous/gateway-*-gateway.log")
    if missing:
...
def validate_run_consistency(loaded_runs: list[tuple[Path, dict[str, Any]]]) -> None:
    ...
    frozen_autoscaling_keys = (
        "enabled", "hpa_name", "api_version", "min_replicas", "max_replicas",
        "target_cpu_utilization", "target_kind", "target_name", "metric",
        "cpu_target_basis", "api_cpu_request", "behavior", "initial_api_replicas",
    )
    autoscaling_values = set()
    for _, artifacts in loaded_runs:
        metadata = artifacts["metadata"]
        source = artifacts["paths"]["metadata"]
        autoscaling = mapping(metadata, "autoscaling", source)
        autoscaling_values.add(canonical_json({key: field(autoscaling, key, source) for key in frozen_autoscaling_keys}))
    if len(autoscaling_values) != 1:
        raise ConsolidationError("configuracao congelada divergente entre execucoes: autoscaling")
    frozen_files = (
        "gateway/effective-config.yaml",
        "kubernetes/manifests/gateway-configmap.yaml",
        "kubernetes/manifests/gateway-deployment.yaml",
    ...
    for filename in frozen_files:
        contents = {(run / filename).read_bytes() for run, _ in loaded_runs}
        if len(contents) != 1:
            raise ConsolidationError(
                f"configuracao congelada divergente entre execucoes: {filename}"
            )
